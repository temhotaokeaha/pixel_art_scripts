...
if [ -d $desiredfolder ]; then
    ...
    spaces=`find . -name '* *' -print -quit`                        # check if files names have spaces
    if [ -n "$spaces" ]; then
        echo "Error: script doesn't support spaces in file names:";
        echo "$spaces";
        echo "Please remove spaces before proceeding.";
    else
        ...
        # magick mogrify -resize 16x16\! temp/*.png;                # resize everything to 16x16. unused
